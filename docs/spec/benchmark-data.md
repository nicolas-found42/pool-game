...
Status: spec data appendix, acquired at #15 (AFK task, no gate), merged into the assembled spec at #12. It feeds `physics.md` §6's ladder; its §6 "supports / does not support" lines are the ladder's data verdicts. Bulk datasets stay outside the repository — every path below is absolute and outside the checkout.
